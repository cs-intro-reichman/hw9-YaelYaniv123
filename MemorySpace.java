...
public class MemorySpace {
    ...
    public void defrag() {
		...
		while (i < freeList.getSize()) {
			MemoryBlock temp = this.freeList.getBlock(i);
			int num = temp.baseAddress + temp.length;
			MemoryBlock temp2 = null;
			int j = 0;
			while (j < freeList.getSize()) {
				if (j == i) {
					j++;
				} else {
					if (freeList.getBlock(j).baseAddress == num) {
						temp2 = freeList.getBlock(j);
						break;
					} else {
						j++;
					}
			...
			if (temp2 != null) {
				freeList.getBlock(i).length += temp2.length;
